...
func setupTest(t *testing.T, votingPowers []VotingPower) (ctx sdk.Context, app *kariapp.kariApp, proposal govv1.Proposal, valoper sdk.AccAddress, voters []sdk.AccAddress) {
	accts := kariapptesting.MakeRandomAccounts(len(votingPowers) + 2)
	deployer := accts[0]
	valoper = accts[1]
	voters = accts[2:]
	...
	// set kari token balance for deployer and voters
	balances := []banktypes.Balance{{
		Address: deployer.String(),
		Coins:   sdk.NewCoins(sdk.NewCoin(kariapp.BondDenom, totalVesting)),
	}}
	for idx, votingPower := range votingPowers {
		balances = append(balances, banktypes.Balance{
			Address: voters[idx].String(),
			Coins:   sdk.NewCoins(sdk.NewCoin(kariapp.BondDenom, sdk.NewInt(votingPower.Staked))),
		})
	}

	app = kariapptesting.MakeMockApp(accts, balances, []sdk.AccAddress{valoper}, sdk.NewCoins())
	...
	instantiateMsg, err := json.karihal(&types.InstantiateMsg{
	...
	for idx, votingPower := range votingPowers {
		if votingPower.Vesting > 0 {
			executeMsg, err := json.karihal(&types.ExecuteMsg{
			...
			_, err = contractKeeper.Execute(
				...
				deployer,
				executeMsg,
				sdk.NewCoins(sdk.NewCoin(kariapp.BondDenom, sdk.NewInt(votingPower.Vesting))),
			)
			require.NoError(t, err)
		}
	}
