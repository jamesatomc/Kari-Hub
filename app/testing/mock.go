...
// MakeMockApp create a mockup `KariApp` instance for testing purposes.
//
// Parameters:
...
//     generated randomly. Each validator does a self-bond of 1_000_000 ukari.
//   - communityPool: Initial balance of the community pool.
func MakeMockApp(accounts []sdk.AccAddress, balances []banktypes.Balance, operators []sdk.AccAddress, communityPool sdk.Coins) *kariapp.KariApp {
	encodingConfig := kariapp.MakeEncodingConfig()

	// create app
	app := kariapp.NewKariApp(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
...
func genesisStateWithValSet(cdc codec.JSONCodec, accounts []sdk.AccAddress, balances []banktypes.Balance, operators []sdk.AccAddress, communityPool sdk.Coins) kariapp.GenesisState {
	// start with the defaut genesis state
	genState := kariapp.DefaultGenesisState(cdc)

	// change default bond denom to ukari
	stakingParams := stakingtypes.DefaultParams()
	stakingParams.BondDenom = kariapp.BondDenom

	// each validator self-bonds 1000000 ukari
	...
	balances = append(
		...
		banktypes.Balance{
			...
			Coins:   sdk.NewCoins(sdk.NewCoin(kariapp.BondDenom, bondAmt.Mul(sdk.NewInt(int64(len(operators)))))),
		},
		banktypes.Balance{
...
func MakeSimpleMockApp() *kariapp.KariApp {
