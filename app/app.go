...
import (
	...
	customgov "hub/x/gov"
	customgovkeeper "hub/x/gov/keeper"
	...
	// kari modules
	"hub/x/incentives"
	incentiveskeeper "hub/x/incentives/keeper"
	incentivestypes "hub/x/incentives/types"
	"hub/x/safety"
	safetykeeper "hub/x/safety/keeper"
	safetytypes "hub/x/safety/types"

	kariwasm "hub/app/wasm"
	karidocs "hub/docs"
)

const (
	AccountAddressPrefix = "kari"
	Name                 = "kari"
...
func init() {
	...
	DefaultNodeHome = filepath.Join(userHomeDir, ".kari")
}
...
// `KariApp` must implement `simapp.App` and `servertypes.Application` interfaces
var (
	_ simapp.App              = (*KariApp)(nil)
	_ servertypes.Application = (*KariApp)(nil)
)

// KariApp extends an ABCI application, but with most of its parameters exported.
...
// NewKariApp creates and initializes a new `KariApp` instance
func NewKariApp(
	...
	// register wasm bindings of Kari modules here
	wasmOpts = append(kariwasm.RegisterCustomPlugins(&app.DistrKeeper), wasmOpts...)
	...
	app.GovKeeper = customgovkeeper.NewKeeper(
		...
		//
		// at Kari we require all proposal metadata to be stored on-chain, and
...
//------------------------------------------------------------------------------
// Implement `kariapptypes.App` interface for KariApp
//
...
func (app *KariApp) Name() string {
	return app.BaseApp.Name()
}

// LegacyAmino returns the app's legacy amino codec
func (app *KariApp) LegacyAmino() *codec.LegacyAmino {
	return app.legacyAmino
}

// BeginBlocker application updates every begin block
func (app *KariApp) BeginBlocker(ctx sdk.Context, req abci.RequestBeginBlock) abci.ResponseBeginBlock {
	return app.mm.BeginBlock(ctx, req)
}

// EndBlocker application updates on every end block
func (app *KariApp) EndBlocker(ctx sdk.Context, req abci.RequestEndBlock) abci.ResponseEndBlock {
	return app.mm.EndBlock(ctx, req)
}

// InitChainer application updates at chain initialization
func (app *KariApp) InitChainer(ctx sdk.Context, req abci.RequestInitChain) abci.ResponseInitChain {
...
func (app *KariApp) LoadHeight(height int64) error {
	return app.LoadVersion(height)
}

// ModuleAccountAddrs return the app's module account addresses
func (app *KariApp) ModuleAccountAddrs() map[string]bool {
...
func (app *KariApp) SimulationManager() *module.SimulationManager {
	return nil
}

//------------------------------------------------------------------------------
// Implement `servertypes.Application` interface for `KariApp`
//
// `RegisterGRPCServer` is already implemented by `BaseApp` and inherited by KariApp
//------------------------------------------------------------------------------

// RegisterAPIRoutes registers all application module routes with the provided
// API server.
func (app *KariApp) RegisterAPIRoutes(apiSvr *api.Server, apiConfig config.APIConfig) {
	...
	if apiConfig.Swagger {
		apiSvr.Router.Handle("/swagger.yml", http.FileServer(http.FS(karidocs.Swagger)))
		apiSvr.Router.HandleFunc("/swagger/", karidocs.Handler(Name, "/swagger.yml"))
	}
}

// RegisterTxService implements the Application.RegisterTxService method.
func (app *KariApp) RegisterTxService(clientCtx client.Context) {
...
func (app *KariApp) RegisterTendermintService(clientCtx client.Context) {
...
func getSubspace(app *KariApp, moduleName string) paramstypes.Subspace {
...
func getBlockedModuleAccountAddrs(app *KariApp) map[string]bool {
...
func initGovRouter(app *KariApp) govv1beta1.Router {
...
func initIBCRouter(app *KariApp) *ibcporttypes.Router {
