...
ifeq (cleveldb,$(findstring cleveldb,$(KARI_BUILD_OPTIONS)))
  build_tags += gcc cleveldb
else ifeq (rocksdb,$(findstring rocksdb,$(KARI_BUILD_OPTIONS)))
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=kari \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=karid \
...
ifeq (,$(findstring nostrip,$(KARI_BUILD_OPTIONS)))
  ldflags += -w -s
endif
...
install: enforce-go-version
	@echo "🤖 Installing karid..."
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/karid
	@echo "✅ Completed installation!"

build: enforce-go-version
	@echo "🤖 Building karid..."
	go build $(BUILD_FLAGS) -o $(BUILDDIR)/ ./cmd/karid
...
containerProtoGenGo=kari-proto-gen-go-$(protoVer)
containerProtoGenSwagger=kari-proto-gen-swagger-$(protoVer)
